class Solution {
    public boolean checkInclusion(String s1, String s2) {
        ...
        for (int i = 0; i + windowLength < s2.length(); i++) {
            ...
            int leftCharCount = windowMap.getOrDefault(leftChar, 0) - 1;
            if (leftCharCount == 0) {
                windowMap.remove(leftChar);
            } else {
                windowMap.put(leftChar, leftCharCount);
            }

            int rightCharCount = windowMap.getOrDefault(rightChar, 0) + 1;
            windowMap.put(rightChar, rightCharCount);
